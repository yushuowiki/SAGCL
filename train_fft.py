...
f = open('NCLA_' + args.dataset + '_fft.txt', 'a+')
f.write('\n\n\n{}\n'.format(args))
f.flush()
...
for epoch in range(args.epochs):
    ...
    with torch.no_grad():
        ...
        if ACC > best_acc:
            torch.save(model.state_dict(), 'best_NCLA_{}_fft.pkl'.format(args.dataset))
...
model.load_state_dict(torch.load('best_NCLA_{}_fft.pkl'.format(args.dataset)))
model.eval()
with torch.no_grad():
    heads, cs = model(features)
